...
public class PreinscripcionesDAO extends DAO{
    ...
    private final String BAJA_PREINSCRIPCION = "DELETE FROM preinscripciones WHERE id_preinscripcion = ?;";
    ...
    public List<Preinscripcion> recuperarPreinscripcionesPorOperador(String dni) throws BaseDeDatosException {
        ...
        try {
          ...
          while(rs.next()) {//Pregunta si hay un registro mas
            ...
            throw new BaseDeDatosException("Error en el consulta de preinscripciones: "+ e.getMessage());
        }
        cerrarConexion(conn);
        return preinscripciones;
    }
    
    public void darDeBajaPreinscripcion(int idPreinscripcion) throws BaseDeDatosException {
        Connection conn = obtenerConexion();
        PreparedStatement prsmt = null;
        
        try {
          prsmt = conn.prepareStatement(BAJA_PREINSCRIPCION);
          prsmt.setInt(1, idPreinscripcion);
          prsmt.execute();
        } catch (SQLException e) {
            throw new BaseDeDatosException("Error en el borrado de preinciscripciones: "+ e.getMessage());
        }
        cerrarConexion(conn);
    }
